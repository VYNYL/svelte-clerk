...
import { AuthStatus, createClerkRequest, TokenType } from '@clerk/backend/internal';
import { parse, splitCookiesString } from 'set-cookie-parser';
import { createCurrentUser } from './currentUser.js';
import { handleNetlifyCacheInDevInstance } from '@clerk/shared/netlifyCacheHandler';
export function withClerkHandler(middlewareOptions) {
    return async ({ event, resolve }) => {
        ...
        // Only set headers/cookies if not a token expiration failure
        const reason = requestState.reason || '';
        const isTokenExpired = reason.includes('token-expired') ||
            reason.includes('session-token-expired') ||
            reason.includes('refresh_expired') ||
            reason.includes('session_token_consumed');
        if (requestState.status === AuthStatus.SignedOut && isTokenExpired) {
            if (debug) {
